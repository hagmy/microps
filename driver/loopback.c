...
struct loopback_queue_entry {
  uint16_t type;
  size_t len;
  uint8_t data[]; /* flexible array number */
};
...
struct net_device *loopback_init(void) {
  ...
  dev->type = NET_DEIVCE_TYPE_LOOPBACK;
  ...
  /* register loopback device and set interrupt handler */
  intr_request_irq(LOOPBACK_IRQ, loopback_isr, lo->irq, dev->name, dev);
